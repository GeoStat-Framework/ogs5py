...
def _stru_point_read(obj):
    ...
    for arr in output["cell_data"]:
        output["cell_data"][arr] = \
            np.squeeze(np.reshape(output["cell_data"][arr],
                                  np.maximum(dim-1, 1),
                                  order="F"))
    for arr in output["point_data"]:
        output["point_data"][arr] = \
            np.reshape(output["point_data"][arr],
                       dim,
                       order="F")
    return output
...
vtkreader_dict = {"unstructured_grid": (vtkUnstructuredGridReader,
                                        _unst_grid_read),
                  "structured_grid": (vtkStructuredGridReader,
                                      _stru_grid_read),
                  "structured_points": (vtkStructuredPointsReader,
                                        _stru_point_read),
                  "polydata": (vtkPolyDataReader,
                               _poly_data_read),
                  "rectilinear_grid": (vtkRectilinearGridReader,
                                       _rect_grid_read)}

XMLreader_dict = {"UnstructuredGrid": (vtkXMLUnstructuredGridReader,
                                       _unst_grid_read),
                  "StructuredGrid": (vtkXMLStructuredGridReader,
                                     _stru_grid_read),
                  "PolyData": (vtkXMLPolyDataReader,
                               _poly_data_read),
                  "RectilinearGrid": (vtkXMLRectilinearGridReader,
                                      _rect_grid_read)}
