...
def check_mesh_dict(mesh, verbose=True):
    ...
    if "PCS_TYPE" in in_mesh_data_keys:
        if isinstance(mesh["mesh_data"]["PCS_TYPE"], STRTYPE) and \
           " " not in mesh["mesh_data"]["PCS_TYPE"]:
            if verbose:
    ...
    if "GEO_TYPE" in in_mesh_data_keys:
        if isinstance(mesh["mesh_data"]["GEO_TYPE"], STRTYPE) and \
           " " not in mesh["mesh_data"]["GEO_TYPE"]:
            if verbose:
    ...
    if "GEO_NAME" in in_mesh_data_keys:
        if isinstance(mesh["mesh_data"]["GEO_NAME"], STRTYPE) and \
           " " not in mesh["mesh_data"]["GEO_NAME"]:
            if verbose:
    ...
    if "LAYER" in in_mesh_data_keys:
        if isinstance(mesh["mesh_data"]["LAYER"], int) and \
           mesh["mesh_data"]["LAYER"] >= 0:
            if verbose:
    ...
    if mesh["nodes"] is None and len(in_element_keys) == 0:
        ...
        if isinstance(mesh["nodes"], np.ndarray) and \
           np.issubdtype(mesh["nodes"].dtype, np.floating) and \
           mesh["nodes"].ndim == 2 and \
           mesh["nodes"].shape[1] == 3:
    ...
    for elem in in_element_keys:
        # see https://github.com/numpy/numpy/pull/9505 for issubdtype
        if isinstance(mesh["elements"][elem], np.ndarray) and \
           np.issubdtype(mesh["elements"][elem].dtype, np.integer) and \
           mesh["elements"][elem].ndim == 2 and \
           mesh["elements"][elem].shape[1] == NODE_NO[elem] and \
           np.min(mesh["elements"][elem]) >= 0 and \
           np.max(mesh["elements"][elem]) < node_cnt:
            if verbose:
            ...
            return False
        # see https://github.com/numpy/numpy/pull/9505 for issubdtype
        if isinstance(mesh["material_id"][elem], np.ndarray) and \
           np.issubdtype(mesh["material_id"][elem].dtype, np.integer) and \
           ...
           np.min(mesh["material_id"][elem]) >= -1:
            ...
            return False
        # see https://github.com/numpy/numpy/pull/9505 for issubdtype
        if isinstance(mesh["element_id"][elem], np.ndarray) and \
           np.issubdtype(mesh["element_id"][elem].dtype, np.integer) and \
           mesh["element_id"][elem].ndim == 1 and \
           mesh["element_id"][elem].shape[0] == \
        ...
        material_id_stack = np.hstack((material_id_stack,
                                       mesh["material_id"][elem]))
        element_id_stack = np.hstack((element_id_stack,
                                      mesh["element_id"][elem]))
    # check ranges of material_id and element_id
    if len(material_id_stack) > 0:
        if set(material_id_stack) == \
           set(np.arange(np.max(material_id_stack)+1)):
            if verbose:
    ...
    if len(element_id_stack) > 0:
        if len(element_id_stack) == no_of_elements(mesh) and \
           len(np.unique(element_id_stack)) == no_of_elements(mesh) and \
           np.min(element_id_stack) == 0 and \
           np.max(element_id_stack) == no_of_elements(mesh)-1:
            if verbose:
