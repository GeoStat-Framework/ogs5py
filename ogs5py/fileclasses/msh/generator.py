...
def rectengular(dim=3,
    ...
    node_arr[:, 0] = (dx
                      * ((np.arange(node_no) % ((x_no+1)*(y_no+1))) % (x_no+1))
                      + x0)
    node_arr[:, 1] = (dy
                      * ((np.arange(node_no) % ((x_no+1)*(y_no+1)))//(x_no+1))
                      + y0)
    ...
    if dim == 2:
        ...
        element_arr[:, 0] = np.arange(element_no)
        element_arr[:, 0] += (x_no+y_no+1)*(np.arange(element_no)//(x_no*y_no))
...
def radial(dim=3,
    ...
    assert (all(rad[i] < rad[i+1] for i in range(len(rad)-1))),\
    ...
    if dim == 2:
        ...
        for ri, re in enumerate(rad):
            for n in range(angles):
                no = n+ri*angles
                node_arr[no, :] = [x0 + re*np.cos(n/angles*2*np.pi),
        ...
        for z in range(z_no-1):
            for r in range(r_no-1):
                for n in range(angles):
                    ...
                    no3 = no4 + angles
                    element_arr[no, :] = [no1, no2, no3, no4,
                                          no1+lay_no,
                                          no2+lay_no,
                                          no3+lay_no,
                                          no4+lay_no]
        ...
        if closed:
            for z in range(z_no-1):
                for n in range(angles):
                    ...
                    elem_mid_arr[no, :] = [no1+lay_no, no2+1, no3+lay_no,
                                           no1, no2, no3]
...
def grid_adapter2D(out_dim=(100., 100.),
    ...
    if fill:
        element_no = [int(in_dim[0]/in_res[0]),
                      int(in_dim[1]/in_res[1])]
        mesh_in = rectengular(dim=2,
                              mesh_origin=in_pos+(z_pos,),
                              element_no=element_no,
                              element_size=in_res)
        mesh_in.MATERIAL_ID = in_mat
...
def grid_adapter3D(out_dim=(100., 100.),
    ...
    geo = gmsh_grid_adapt3D(out_dim, in_dim, z_dim,
                            out_res, in_res,
                            out_pos, in_pos, z_pos)
    points, cells, __, __, __ = pg.generate_mesh(geo)
    out = convert_meshio(points, cells, import_dim=3)
    out["material_id"] = gen_std_mat_id(out["elements"], out_mat)
    ...
    if fill:
        ...
        mesh_in = rectengular(dim=3,
                              mesh_origin=in_pos+(z_pos+min(z_dim, 0.),),
                              element_no=element_no,
                              element_size=in_res)
        mesh_in.MATERIAL_ID = in_mat
...
def generate_gmsh(path_or_code, import_dim=(1, 2, 3)):
    ...
    points, cells, __, __, __ = pg.generate_mesh(geo)
    out = convert_meshio(points, cells, import_dim=import_dim)
    return out
